...
const AddService = () => {
    ...
    const handleAddService = async event => {
        ...
        try {
            const url = '/services';
            const response = await axiosSecure.post(url, newService);
            if (response.status === 200) {
                Swal.fire('Success!', 'Add Service completed!', 'success');
            } else {
                Swal.fire('Error', 'Add Service failed', 'error');
            }
        } catch (error) {
            Swal.fire('Error', 'Add Service failed', 'error');
        }
            ...
            <div className="w-full max-w-md mx-auto">
              <div className="py-10 px-14 bg-[#D7CBA3] rounded-md">
                <div className="text-center my-5">
                  <h1 className="mb-6 text-[#a55e3f] font-garamond uppercase font-semibold text-3xl">Add New Service</h1>
                  <p className="text-gray-700">You can add new services from here.</p>
                </div>
